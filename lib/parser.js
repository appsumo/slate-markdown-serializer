...
Lexer.prototype.token = function (src, top, bq) {
  ...
  while (src) {
    ...
    if (cap = this.rules.heading.exec(src)) {
      ...
      // center
      if (cap = this.rules.center.exec(_text)) {
        _text = _text.substring(cap[0].length);
        _text = "" + cap[1].trim() + _text;
        _align = "center";
      }
    ...
    if (top && (cap = this.rules.paragraph.exec(src))) {
      ...
      var _align3 = void 0;
      var _text2 = cap[1].charAt(cap[1].length - 1) === "\n" ? cap[1].slice(0, -1) : cap[1];

      // center
      if (cap = this.rules.center.exec(_text2)) {
        _text2 = _text2.substring(cap[0].length);
        _text2 = "" + cap[1].trim() + _text2;
        _align3 = "center";
      }
...
Renderer.prototype.groupTextInLeaves = function (childNode) {
  ...
  var out = node.reduce(function (acc, current) {
    ...
    if (current instanceof TextNode) {
      if (lastIsText) {
        // If the previous item was a text object, push the current text to it's leaf
...
Parser.prototype.tok = function () {
  switch (this.token.type) {
    ...
    case "heading":
      {
        return this.renderer.heading(this.inline.parse(this.token.text), this.token.depth, this.token.align);
      }
    case "block-code":
      {
        return this.renderer.blockcode(this.inline.parse(this.token.text), this.token.lang, this.token.align);
      }
    case "table":
      {
        var body = [];
    ...
    case "blockquote_start":
      {
        ...
        while (this.next().type !== "blockquote_end") {
          ...
          _body.push(this.token.type === "text" ? this.parseText() : this.tok());
        }
        return this.renderer.blockquote(_body, align);
      }
    case "list_start":
      {
        var _body2 = [];
        ...
        return this.renderer.list(_body2, style, start);
      }
    ...
    case "paragraph":
      {
        return this.renderer.paragraph(this.inline.parse(this.token.text), this.token.align);
      }
    case "text":
      {
